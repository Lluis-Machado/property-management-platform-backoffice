...
const FixedAssetsPage = ({
    ...
    const [depreciationPopupVisibility, setDepreciationPopupVisiblity] =
        useState<PopupVisibility>({ hasBeenOpen: false, visible: false });
    const [invoicePopupVisibility, setInvoicePopupVisibility] =
        useState<PopupVisibility>({ hasBeenOpen: false, visible: false });
    ...
    const handleDepreciationClick = useCallback(
        (name: string, depreciation: any[]) => {
    ...
    return (
        <>
            <DataGrid
                ...
                years={years}
            />
            {(depreciationPopupVisibility.visible ||
                depreciationPopupVisibility.hasBeenOpen) && (
                <Popup
                    data={selectedFixedAsset}
                    isVisible={depreciationPopupVisibility.visible}
                    onClose={() =>
                        setDepreciationPopupVisiblity((p) => ({
                            ...p,
                            visible: false,
                        }))
                    }
            ...
            {(invoicePopupVisibility.visible ||
                invoicePopupVisibility.hasBeenOpen) && (
                <PopupPreview
                    fileURL={selectedInvoice.url}
                    isVisible={invoicePopupVisibility.visible}
                    onClose={() =>
                        setInvoicePopupVisibility((p) => ({
                            ...p,
                            visible: false,
                        }))
                    }
