...
async function loginRoute(req: NextApiRequest, res: NextApiResponse) {
    ...
    console.log(
        'LLAMANDO A URL: ',
        `${process.env.NEXT_PUBLIC_API_GATEWAY_URL}/auth/Token/GetBackofficeToken?username=${username}&password=${password}`
    );
    ...
    const userRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_GATEWAY_URL}/auth/Users/${decoded.sub}`,
        {
            method: 'GET',
            headers: {
    ...
    const { user_id, email, name, nickname, picture }: Auth0User =
        await userRes.json();
